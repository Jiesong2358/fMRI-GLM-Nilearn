...
for sub in subject_range:
    output_dir = 'S:/GVuilleumier/GVuilleumier/groups/jies/Spatial_Navigation/Final_data/GLM/sub-{:0>2d}'.format(sub)
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
        # if the subject number =1,2,3,4, skip the current loop
    if sub in [1,2,3,4,6,11,13,14,23]:
        continue
    # Set root directories
    root_dir = 'S:/GVuilleumier/GVuilleumier/groups/jies/Spatial_Navigation/Final_data/nifti/derivatives/fmriprep'
    events_root_dir = 'S:/GVuilleumier/GVuilleumier/groups/jies/Spatial_Navigation/Final_data/nifti'
    # Load template and set TR
    template = 'S:/GVuilleumier/GVuilleumier/groups/jies/Spatial_Navigation/Final_data/Atlas/tpl-MNI152NLin2009cAsym_res-01_desc-brain_T1w.nii.gz'
    tr = 1.3
    ...
    for run in range(1, 7):
        run_file = os.path.join(root_dir, 'sub-{:0>2d}/func/sub-{:0>2d}_task-rep_run-{}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz'.format(sub, sub, run))
        ...
        events_file = os.path.join(events_root_dir,'sub-{:0>2d}/func/sub-{:0>2d}_task-rep_ret_run-{:0>2d}_response.tsv'.format(sub, sub, run))
        events_data = pd.read_csv(events_file, sep="\t")
        confound_file = os.path.join(root_dir,'sub-{:0>2d}/func/sub-{:0>2d}_task-rep_run-{}_desc-confounds_timeseries.tsv'.format(sub, sub, run))
        ...
        output_file = os.path.join(output_dir, 'Interesction_design_matrix_run{:0>2d}.png'.format(run))
    ...
    # Define the contrasts
    contrasts = {
        'sub-{:0>2d}_Resp_Car_I1'.format(sub): pad_vector([1, 0, -1, 0],n_columns),
